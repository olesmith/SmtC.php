...
class HtmlInput extends HtmlSelect
{
    ...
    var $HtmlInput_Classes='myinputs input';
...
function MakeButton($type,$title,$options=array())
{
    ...
    /* $options[ "TYPE" ]=$type; */
    /* $html="<BUTTON".$this->Htmls_Options_String($options).">$title</BUTTON>\n"; */
...
function MakeInput($name,$value="",$size=10,$options=array())
{
    ...
    return $this->Htmls_Input_Text($name,$value,$options);
    ...
    /* $classes=array($this->HtmlInput_Classes,$this->Bulma_Size); */
    ...
    /* return "<INPUT".$this->Htmls_Options_String($options).">"; */
}
...
function MakePassword($name,$value,$size=8,$maxsize=0,$options=array())
{
    ...
    $options[ "SIZE" ]=$size;
    $options[ "CLASS" ]=
        array($this->HtmlInput_Classes,$this->Bulma_Size);
...
function MakeFileField($name,$options=array())
{
    ...
    return "<INPUT TYPE='FILE' NAME='".$name."'".$this->Htmls_Options_String($options).">";
}
...
function HtmlDateInputValue($name,$search=FALSE,$default=TRUE)
{
    ...
    if ($default && empty($date))
    {
        ...
        (
           "%02d/%02d/%d",
           $this->MyTime_Current_Date(),
           $this->MyTime_Current_Month(),
           $this->MyTime_Current_Year()
        );
        
        $year=$this->MyTime_Current_Year();
        $mon=$this->MyTime_Current_Month();
        $day=$this->MyTime_Current_Date();
    }
