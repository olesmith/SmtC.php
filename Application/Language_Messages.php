...
class Language_Messages extends Language_Messages_DBs
{
    ...
    function __construct($args=array())
    {
        ...
        $this->MyMod_Paging_NPages_In_Menu=
            $this->MyMod_Paging_NPages_Intermediate=40;
        
        $this->IncludeAllDefault=False;

        $this->CellMethods[ "Message" ]=TRUE;
        $this->CellMethods[ "Languages_Cell_Info" ]=TRUE;
    ...
    function Languages_Cell_Info($edit=0,$item=array(),$data="")
    {
        if (empty($item))
        {
            return "Info";
        ...
        {
            $info="Data";
            $def=$this->$moduleobj()->ItemData($item[ "Message_Key" ]);
        }
        elseif ($item[ "Message_Type" ]==$this->Language_Action_Type)
        {
            $info="Action";
            $def=$this->$moduleobj()->Actions($item[ "Message_Key" ]);
        }
        elseif ($item[ "Message_Type" ]==$this->Language_Group_Type)
        {
            $info="Group";
            $def=$this->$moduleobj()->ItemDataGroups($item[ "Message_Key" ]);
        }
        elseif ($item[ "Message_Type" ]==$this->Language_SGroup_Type)
        {
            $info="SGroup";
            $def=$this->$moduleobj()->ItemDataSGroups($item[ "Message_Key" ]);
        }
        ...
        if ($def)
        {
            ...
            if (!empty($def[ "Files" ]))
            {
                $deffiles=array_merge($deffiles,$def[ "Files" ]);
            }
            ...
            if (!empty($def[ "File" ]))
            {
                ...
                {
                    $deffiles=array_merge($deffiles,$def[ "File" ]);
                }
                else
                {
                    array_push($deffiles,$def[ "File" ]);
                }
            }
        ...
        if (!empty($def[ $key ]))
        {
            if (is_array($def[ $key ]))
            {
                $files=array_merge($files,$def[ $key ]);
            }
            else
            {
                array_push($files,$def[ $key ]);
            }
        }
        $key="Handler";
        if (!empty($def[ $key ]))
        ...
        if (!empty($def[ $key ]))
        {
            array_push($files,"Singular");
        }
        else
        {
            array_push($files,"Plural");
        }
        ...
        foreach ($valuedatadefs as $data => $def)
        {
            ...
            foreach ($this->Language_Keys() as $lkey)
            {
                ...
                if ($search)
                {
                    ...
                    $this->ItemData[ $rdata ][ "SearchName_DK" ]=$lkey;
                }
