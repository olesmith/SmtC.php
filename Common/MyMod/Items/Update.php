...
trait MyMod_Items_Update
{
    ...
    //* from ItemTable form, using MyMod_Data_Field_Test to test
    ...
    function MyMod_Items_Update($items=array(),$udatas=array(),$postprocess=TRUE)
    {
        ...
        $this->MyMod_Log_Entry(count($items)." items");
        ...
        foreach ($ids as $id)
        {
            ...
            foreach ($udatas as $key)
            {
                ...
                elseif (preg_match('/^FILE$/',$this->ItemData[ $key ][ "Sql" ]))
                {
                    ...
                    if (is_array($res) && $res[ "__Res__" ])
                    {
                        ...
                        {
                            var_dump($this->MyMod_Data_Trigger_Function($key));
                            $ritems[ $id ]=
                                $this->MyMod_Data_Trigger_Apply
                                (
                                    $key,
                                    $ritems[ $id ],
                ...
                {
                    $newvalue=$this->MyMod_Data_Field_Test($key,$ritems[ $id ],TRUE);
            ...
            if ($update>0)
            {
                ...
                {
                    $this->PostUpdateActions($olditem,$ritems[$id],$updatedatas);
                }
