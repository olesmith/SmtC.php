...
trait MyMod_Items_Dynamic_Entry
{
    ...
    function MyMod_Item_Dynamic_Entry($item,$group,$action,$def)
    {
        ...
        $entry=
            ...
            (
                ...
                "HREF" => $this->MyMod_Item_Dynamic_Entry_Url($item,$action,$def),
    ...
    function MyMod_Item_Dynamic_Entry_Args($item,$action,$def)
    {
        ...
        //var_dump($args,$def);
        if (!empty($def[ "GETs" ]) && is_array($def[ "GETs" ]))
        {
        ...
        if (!empty($def[ "Hash" ]) && is_array($def[ "Hash" ]))
        {
            foreach ($def[ "Hash" ] as $data => $rdata)
            {
                ...
                elseif (!empty($def[ "Hash" ][ $data ]))
                {
                    $args[ $data ]=$def[ "Hash" ][ $data ];
                }
            }
        }        
