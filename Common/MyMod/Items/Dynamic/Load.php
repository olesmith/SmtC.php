...
trait MyMod_Items_Dynamic_Load
{
    ...
    function MyMod_Items_Dynamic_Loads_JS($edit,$items,$group)
    {
        ...
        if
            (
                !empty($load_method)
                ||
                !empty($this->CGI_GETint("Include_ID"))
            )
        {
            array_push
            (
    ...
    //*
    //* Detect Load_Action from $group def, may be overwritten.
    //*

    function MyMod_Items_Dynamic_Load_Action($group)
    {
        return
    ...
    function MyMod_Items_Dynamic_Load($load_method,$edit,$item,$items,$group,$action=False,$load=True)
    {
        ...
        $load_action=
            $this->MyMod_Items_Dynamic_Load_Action($group);
            ...
            )
        {
            ...
            {
                $load=$this->$load_method($item,$items);
            }
        }
        ...
        if ($load)
        {
            ...
            if (!empty($element_id))
            {
                ...
                (
                    $js,
                    $this->JS_Click_Element_By_ID($element_id)
                );
            }
        }
