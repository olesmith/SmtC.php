...
trait MyMod_Item_PostProcess
{
    ...
    function MyMod_Item_PostProcess_Value_Get($item,$data,$destdata)
    {
        ...
        {
            $this->MyMod_Item_PostProcess_Values
                [ $data ][ $destdata ]=array();
            
            $this->MyMod_Item_PostProcess_Values
                [ $data ][ $destdata ][ "Names" ]=array();
        }
        ...
        if (empty($this->MyMod_Item_PostProcess_Values[ $data ][ $destdata ][ $item[ $data ] ]))
        {
                ...
                )
            {
                ...
                $this->MyMod_Item_PostProcess_Values
                    [ $data ][ $destdata ][ "Names" ][ $value ]=$name;
            }
        }
        
        return
            $this->MyMod_Item_PostProcess_Values
            [ $data ][ $destdata ][ $item[ $data ] ];
    }
    ...
    function MyMod_Item_PostProcess_Files($item)
    {
        ...
        $uploadpath=$this->MyMod_Data_Upload_Path($item);
        ...
        foreach ($datas as $data)
        {
            if (!empty($item[ $data ]))
            {
                ...
                if ($item[ $data ]!=$fname)
                {
                    ...
                    {
                        $comps=preg_split('/Uploads/',$item[ $data ]);
                        if (count($comps)>=2)
                        {
                            $comps[0]="Uploads";
                            $item[ $data ]=join("/",$comps);
    ...
    //*
    //* function MyMod_Item_PostProcess, Parameter list: $item,$force=False
    //*
    ...
    function MyMod_Item_PostProcess($item,$force=False)
    {
        if (
            !isset($item[ "ID" ])
            ||
        ...
        //$ritem=$this->MyMod_Data_Fields_Enums_ApplyAll($item);
        $updatedatas=array();
