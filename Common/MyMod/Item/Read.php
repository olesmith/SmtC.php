...
trait MyMod_Item_Read
{
    ...
    function MyMod_Item_Read($id,$datas=array(),$noderiveds=FALSE,$updatesitemhashes=TRUE,$postprocess=TRUE)
    {
        ...
        if (empty($id)) { $id=$this->CGI_VarValue("ID"); }

        $where=
            $this->MyMod_Items_Where_Clause_Real
            (
                $this->MyMod_Item_Read_Where($id)
            );

        $this->ItemHash=
            $this->MyMod_Item_Read_Try($id,$rdatas);
        ...
        foreach ($datas as $n => $data)
        {
                ...
                )
            {
                ...
                {
                    $this->ItemHash[ $data ]=$this->ItemData[ $data ][ "Default" ];
    ...
    //*
    //* Where clause for reading single item.
    //* Meant to be overridden on demand.
    //*

    function MyMod_Item_Read_Where($id)
    {
        if (empty($id))
        {
            $id=$this->MyMod_Handle_Item_ID_CGI();
        }

        $where=array();
        if (!empty($id))
        {
            $where[ $this->IDWhereVar ]=$id;
        }
    ...
    //*
    //* Tries to read item by ID => $id. [
    //* Supposed to be overridden.
    //*

    function MyMod_Item_Read_Try($id,$datas=array())
    {
        $where=$this->MyMod_Item_Read_Where($id);
        if (!empty($where))
        {
            $this->ItemHash=
