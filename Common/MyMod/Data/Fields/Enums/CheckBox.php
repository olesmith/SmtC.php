...
trait MyMod_Data_Fields_Enums_CheckBox
{
    ...
    function MyMod_Data_Field_Enum_CheckBox($data,$value,$item,$rdata)
    {
        $checkbox=$this->MyMod_Data_Field_Enum_CheckBox_Is($data);
        
        $options=$this->MyMod_Data_Field_Enum_Options($data,$tabindex=1,$item);
        
        $values=array();            
        $names=array();
        foreach ($this->ItemData($data,"Values") as $no => $name)
        {
            array_push($values,$no+1);
