...
trait MyMod_Data_Fields_Is
{
    ...
    function MyMod_Data_Field_Is_Search($data)
    {
        ...
        if
            (
                ...
                ||
                empty($this->ItemData[ $data ][ "Search_".$this->Profile() ])
            )
        {
    ...
    function MyMod_Data_Field_Files_Get()
    {
        $fields=array();
        foreach (array_keys($this->ItemData) as $field)
        {
            if ($this->MyMod_Data_Field_Is_File($field))
            {
    ...
    function MyMod_Data_Field_Is_Date($data)
    {
        ...
        {
            var_dump("Empty SQL: ".$this->ModuleName,$data,$this->ItemData[ $data ]);
            exit();
        }
        ...
        if (
              ...
              !empty($this->ItemData[ $data ][ "Sql" ])
              &&
              preg_match('/(BIG)?INT/',$this->ItemData[ $data ][ "Sql" ])
              &&
    ...
    {
        if (empty($this->ItemData($data,"SqlClass"))) { return False; }
        
        return $this->ItemData($data,"SqlClass");
    }
    
    //*
    //* Returns item dayas that are Module type.
    //*

    function MyMod_Data_Fields_Is_Module()
    {
        $datas=array();
    ...
    function MyMod_Data_Field_Is_Unique($data)
    {
        ...
        if (
              isset($this->ItemData[ $data ])
              &&
              !empty($this->ItemData[ $data ][ "Unique" ])
           )
        {
            $res=TRUE;
        }
    ...
    //*
    //* Returns all SQL type data for module.
    //*

    function MyMod_Data_Fields_Is_Sql()
    {
        $datas=array();
        foreach (array_keys($this->ItemData) as $data)
        {
    ...
    function MyMod_Data_Field_Sql_2_Module($data)
    {
        return $this->ItemData[ $data ][ "SqlClass" ];
    }
