...
trait MyMod_Data_Fields_File_Decorator_Download
{
    ...
    function MyMod_Data_Fields_File_Decorator_Download_Link($edit,$item,$data,$value)
    {
        if ($this->MyMod_Data_Value_Image_Is($item,$data))
        {
            return
                $this->Htmls_HRef
                (
                    $this->MyMod_Data_Fields_File_Decorator_Download_URI($item,$data),
        ...
        elseif (!empty($value))
        {
            ...
            if (file_exists($file))
            {
               $icon=
                   $this->HRef
                   (
                       $this->MyMod_Data_Fields_File_Decorator_Download_URI
                       (
                           $item,$data
                       ),
                       ...
                       (
                           ...
                           (
                               $item [ $data."_OrigName" ],
                               $this->MyTime_Current_Date_Text( filemtime($file) ).
                               " ".
                               $this->MyTime_Time( filemtime($file) ),
                           )
                       )
    ...
    function MyMod_Data_Fields_File_Decorator_Download_URI($item,$data)
    {
