...
trait MyMod_Data_Fields_File_Decorator
{
    ...
    function MyMod_Data_Fields_File_Decorator($data,$item,$plural=FALSE,$edit=0)
    {
        ...
        $this->Sql_Select_Hash_Datas_Read($item,array($data."_Time",$data."_OrigName"));
        ...
        if (!empty($value))
        {
            ...
            if (!empty($filetime))
            {
                ...
                (
                    $values,
                    $this->MyMod_Data_Fields_File_Decorator_Unlink_SPAN
                    (
                        $edit,$item,$data,$value
                    ),
                    $this->MyMod_Data_Fields_File_Decorator_Download_Link
                    (
                        $edit,$item,$data,$value
                    )
                );
    ...
    function MyMod_Data_Fields_File_Decorator_Latex($data,$item,$plural=FALSE)
    {
        ...
        if (!empty($item[ $data ]))
        {
            ...
            {
                $destfile=
                    $this->MyMod_Data_Upload_FileName_Get($data,$item,"pdf");
        ...
        return
            ...
            (
                ...
                (
                    ...
                    $this->MyLanguage_GetMessage
                    (
                        "File_Original").": ".$item[ $data."_OrigName" ],
                    "Time: ".
                    $this->MyMod_Data_Fields_File_Decorator_Download_TimeStamp
                    (
                        $item,$data
                    ),
                    "Size: ".
                    $this->MyMod_Data_Fields_File_Decorator_SizeInfo
                    (
                        $item,$data
                    ),
                    "Dimensions: ".
                    $this->MyMod_Data_Fields_File_Image_Dimensions
                    (
                        $item,$data
                    )
                )
