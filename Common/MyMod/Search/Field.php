...
trait MyMod_Search_Field
{
    ...
    function MyMod_Search_Field_Make($data,$fixedvalues=array(),$rval="")
    {
        return
    ...
    function MyMod_Search_Field_Value($data,$fixedvalues,$rval="")
    {
        ...
        {
            $value=
                $this->MyMod_Search_Field_Method_Call($data,$fixedvalue,$rval);
        }
        elseif ($this->MyMod_Data_Field_Is_Date($data))
        {
            $value=
                $this->MyMod_Search_Field_Date($data,$rdata,"");
        }
        elseif ($this->MyMod_Data_Field_Is_Time($data))
        {
            $value=
                $this->MyMod_Search_Field_Time($data,$rdata,$rval);
        }
        elseif ($this->MyMod_Data_Field_Is_Enum($data))
        {
            $value=
                $this->MyMod_Search_Field_Enum($data,$rdata,$rval);
        }
        elseif ($this->MyMod_Data_Field_Is_Sql($data))
        {
            $value=
                $this->MyMod_Search_Field_Sql($data,$rdata,$rval);
        }
        elseif ($this->MyMod_Data_Field_Is_File($data))
        {
            $value=
                $this->MyMod_Search_Field_File($data,$rdata,$rval);
        }
        else
        {
