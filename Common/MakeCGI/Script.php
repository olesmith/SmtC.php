...
trait MakeCGI_Script
{
    ...
    //*
    //* Path sent to cookies. 
    //*
    
    function CGI_Cookie_Path()
    {
        return $this->CGI_Script_Path();
    }

    function CGI_Request_URI()
    {
    ...
    function CGI_Script_Full_Path($hide_index_php=True)
    {
        $exec=
        ...
        if ($hide_index_php)
        {
            $exec=preg_replace('/index.php/',"",$exec);
        }
    ...
    function CGI_Script_Url($hash,$target="")
    {
        #if ($scriptname=="") { $scriptname=$this->CGI_Script_Name(); }

        if (!empty($target)) { $target="$#".$target; }
