...
trait MakeCGI_Cookies
{
    ...
    function MakeCGI_Cookie_Set($name,$value="",$expire=0)
    {
        ...
        if ($this->Handle && $this->CookiesWritten==0)
        {
            ...
            {
                $this->MakeCGI_Cookie_Do_Set($name,$value,$expire);
    ...
    //*
    //* Calls setcookie: php-verdsion dependent..
    //*

    function MakeCGI_Cookie_Do_Set($name,$value,$expire)
    {
        //var_dump($phpver);
        $cookie_path=$this->ApplicationObj()->CGI_Cookie_Path();
        if ($this->PHP_Version_Float()<7.3)
        {
            $cookie_path="/; samesite=strict";
            $res=
                setcookie
    ...
    //*
    //* Float value of php version (form comparison).
    //*

    function PHP_Version_Float()
    {
        $comps=preg_split('/[\.-]/',phpversion());

        $phpver=7.0;
        if (count($comps)>=3)
        {
            $phpver=floatval($comps[0].".".$comps[1].$comps[2]);
        }
