...
trait CallStack
{
    ...
    function CallStack_Callers_Infos($sep="\n",$parm=FALSE)
    {
        $trace=debug_backtrace($parm);

        $infos=array();
        for ($level=1;$level<count($trace);$level++)
        {
            array_push
            (
                $infos,
                $this->CallStack_Callers_Info($level,$parm)
            );
        }

        return join($sep,$infos);
    }
    ...
    function CallStack_Callers_Info($level=3,$parm=FALSE)
    {
        $trace=debug_backtrace($parm);
        if (True)//count($trace[ $level ])>=$level )
        {
            return
    ...
    //*
    //* Returns caller $level info.
    //*

    function CallStack_Caller_Info($level=1)
    {
        $trace=$this->CallStack_Caller($level);
    ...
    function CallStack_Show($parm=FALSE)
    {
        ...
        {
            echo
                join("\t",$this->CallStack_Trace_TitleRow($trace));
        ...
        else
        {
            ...
            (
                array
                (
                    $this->H(3,"CallStack".$callerlevel),
                    $this->Htmls_Table
                    (
                        $this->CallStack_Trace_TitleRow($trace),
                        $table
                    ),
                )
            );
