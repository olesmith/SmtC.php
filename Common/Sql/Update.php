...
trait Sql_Update
{
    ...
    function Sql_Update_Item($item,$where,$datas=array(),$table="",$force=False,$pretend=False)
    {
        ...
        if ($this->MyMod_Sql_Update_Log)
        {
            $this->Sql_Update_Item_Log($item,$query);
        }
    ...
    //*
    //* Updates $item with column $data - if $item unique, according to $where!"
    //* 

    function Sql_Update_Set_Column_Where_IS_NULL($data,$value,$table="",$force=False)
    {
    ...
    //*
    //* Logs update if activated or module.
    //* 

    function Sql_Update_Item_Log($item,$query)
    {
        $time=
            $this->MyTime_Curent_Date_Comps().".".
            sprintf("%02d",$this->MyTime_Current_Hour());
        
        $file=
            "/usr/local/SigaZ/Logs/".$this->ModuleName.".".$time.".log";

        $user="public";
        if ($this->LoginData())
        {
            $user=
                $this->LoginData("Email")." ".$this->Profile();
        }
