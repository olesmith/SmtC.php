...
{    
    ...
    {
        if (empty($datadef[ "Sql" ]))
        {
            var_dump("Warning! ".$data.", Add: Sql key is empty! File: ",$datadef[ "File" ],$datadef);
            return;
        }
        ...
        if (!empty($sqltype))
        {
            ...
            $this->ApplicationObj()->AddPostMessage
            (
                "Add Column ".$data." in $table: $sqltype<BR>"
            );

            if ($default)
            {
                $this->Sql_Update_Set_Column_Where_IS_NULL
                (
                    $data,$default,$table,
                    $force=True //necessary
                );
            }
        }
        else
        {
            $this->ApplicationObj()->AddPostMessage
            (
                "Add Column ".$data." in $table, no SQL type: $sqltype<BR>"
            );
        }
    }
