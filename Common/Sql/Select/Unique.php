...
trait Sql_Select_Unique
{
    ...
    function Sql_Select_Unique_Col_Values($cols,$where=array(),$orderby="",$table="",$debug=False)
    {
        $singlecol=FALSE;
        if (!is_array($cols))
        {
        ...
        $query=$this->Sql_Select_Unique_Col_Values_Query($cols,$where,$orderby,$table,$debug);
    ...
    function Sql_Select_Unique_Col_N($cols,$where=array(),$orderby="",$table="")
    {
        $values=
            $this->Sql_Select_Unique_Col_Values($cols,$where,$orderby,$table);
    ...
    function Sql_Select_Unique_Col_Values_Query($cols,$where=array(),$orderby="",$table="",$debug=False)
    {
        if (!is_array($cols)) { $cols=array($cols); }
        
        if (!$this->Sql_Table_Exists($table)) { return ""; }

        if (is_array($where)) { $where=$this->Hash2SqlWhere($where,"",$debug); }
   ...
   //*
    ...
    function Sql_Select_Unique_Col_Last_Value($col,$where=array(),$orderby="",$table="")
    {
        $values=
   ...
   //*
    ...
    function Sql_Select_Unique_Item($where,$datas=array(),$delete=FALSE,$table="")
    {
        ...
        if (count($items)>0)
        {
            ...
            (
                "Sql_Select_Unique_Item: non-unique items"
            );
