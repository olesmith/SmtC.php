...
trait MyEmail 
{
    ...
    function MyEmail_Email_Headers_Set($mailhash)
    {
        ...
        foreach ($mailhash[ "To" ] as $id => $to)
        {
            ...
            $this->Email_PHPMailer->AddAddress($to);
        }
        ...
        foreach ($mailhash[ "ReplyTo" ] as $id => $rto)
        {
            if (empty($rto)) { continue; }
    ...
    function MyEmail_Email_Send($setup,$mailhash,$attachments=array())
    {
        ...
        $mailhash[ "Body" ]=html_entity_decode($mailhash[ "Body" ],ENT_IGNORE);
        $mailhash[ "Subject" ]=html_entity_decode($mailhash[ "Subject" ],ENT_IGNORE);

        if (!empty($mailhash[ "FromName" ]))
        {
            $mailhash[ "FromName" ]=utf8_decode($mailhash[ "FromName" ]);
        }
        ...
        {
            $res=
                $this->Email_PHPMailer->addAttachment
                (
                    $attachment[ "File" ],
                    $attachment[ "Name" ]
                );
        ...
        $this->Email_PHPMailer->WordWrap = 50; // set word wrap
        $this->Email_PHPMailer->Priority = 1; 
        $this->Email_PHPMailer->IsHTML(false);
        $this->Email_PHPMailer->SMTPDebug=0;
