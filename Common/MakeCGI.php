...
trait MakeCGI
{
    ...
    function MakeCGI_CLI_Is()
    {
        return !$this->MakeCGI_CGI_Is();
    }
    
    function MakeCGI_CGI_Is()
    {
        $res=False;
    ...
    function CGI_Reset_Cwd()
    {
        ...
        //Make sure we are reading source dirs for setup
        //Needed for Uploads to work.
    ...
    //*
    //* sub CGI_COOKIEint, Parameter list: $name
    //*
    //* Returns $_COOKIE $name key as int.
    //*

    function CGI_COOKIEint($name)
    {
        $value="";
        if (isset($_COOKIE[ $name ]))
        {
    ...
    //*
    //* sub CGI_VarValueint, Parameter list: $name
    //*
    //*
    //*

    function CGI_VarValueint($name)
    {
        return
            $this->CGI_Treatint
            (
                $this->CGI_VarValueint($name)
            );
    }
    ...
    function CGI_GET2Hash($cgivar,$obj,$key="",$attrname="",$die=TRUE,$warn=FALSE,$default=0)
    {
        if (!$this->MakeCGI_CGI_Is())
        {
            return NULL;
        ...
        if (empty($this->$attrname))
        {
            ...
            if (empty($id) && !empty($default)) { $id=$default; }
            ...
            foreach ($this->CGI_Split_Vars as $var => $def)
                {
                    ...
                    if ($val!="")
                    {
                        ...
                        (
                            $fields,
                            $this->MakeHidden($vvar."_Only",$val)
                        );
                    }
                }
        }
    ...
    function CGI_CommonArgs_Add(&$args)
    {
        if (is_string($args))
        {
            $args=$this->CGI_Query2Hash($args);
        }
        
        if (is_array($this->URL_CommonArgs))
        {
            $args=
                array_merge
                (
    ...
    //*
    //* Trims input $item. Backslashes and suff.
    //*

    function CGI_Trim_Hash(&$item)
    {
        foreach (array_keys($item) as $data)
        {
            $item[ $data ]=
                $this->CGI_Trim_Value($item[ $data ]);
                    
        }
    }
    
    //*
    //* Trims input value. Backslashes and suff.
    //*

    function CGI_Trim_Value($value)
    {
        //replace's
        $value=preg_replace("/'/","&#39;",$value);
