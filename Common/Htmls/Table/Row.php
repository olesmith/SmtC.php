...
trait Htmls_Table_Row
{
    ...
    function Htmls_Table_Row($row,$options=array(),$tdoptions=array(),$tdtag="TD",$count=0)
    {
        ...
        for ($n=0;$n<count($row);$n++)
        {
            if (isset($row[$n]))
            {
                ...
                //var_dump($this->Htmls_Table_Cell($row[$n],$rtdoptions,$tdtag));
                array_push
                (
                    $html,
    ...
    //*
    //* Row options: odd/even
    //*

    function Htmls_Table_Row_Options_Odd_Even($n)
    {
        return
            array
            (
                "ONMOUSEOVER" => "Highlight_TR(this,'lightgray');",
                "ONMOUSEOUT"  => "Highlight_TR(this);",
                "STYLE"       => $this->Htmls_Table_Row_Style_Odd_Even($n),
            );
    }
    ...
    function Htmls_Table_Row_Style_Odd_Even($n)
    {
        $style=array();
        
        $nn=($n% count($this->MyMod_Items_Dynamic_Table_Colors));

        $style[ "background-color" ]=
            $this->MyMod_Items_Dynamic_Table_Colors[ $nn ];
