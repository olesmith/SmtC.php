...
trait MyLatex
{
    ...
    function Latex_Run($texfilename,$latex,$runbibtex=FALSE)
    {
        ...
        $outfile=preg_replace('/\.pdf/',".out",$pdffilename);

        $this->UnlinkFiles
        (
            array
            (
                $auxfile,$tocfile,$outfile,"latex.log"
            ),
            $path
        );
    ...
    //*
    function Latex_PDF($file,$latex, $printpdf = true, $runbibtex = false, $copypdfto = false, $clean = true, $exit = true,$printtex=True)

#    function Latex_PDF($texfilename,$latex,$printpdf=TRUE,$runbibtex=FALSE,$copypdfto=FALSE,$clean=True,$exit=True,$printtex=True)
    {
        $texfilename=preg_replace('/&quot;/',"",$file);
        $texfilename=$this->Latex_File_Name($texfilename);
        ...
        if ($pdffilename && is_file($rpdffilename))
        {
            if ($printpdf)
            {
                ...
                (
                    "pdf",
                    preg_replace('/^_tmp_/',"",$pdffilename)
                );
                ...
                elseif ($clean)
                {
                    ...
                    (
                        ...
                        (
                            $path."/".$pdffilename,
                            $path."/".
                            preg_replace('/\.tex$/',".log",$texfilename),
                            $path."/".
                            preg_replace('/\.tex$/',".out",$texfilename)
                        )
                    );
                }
    ...
    function Latex_Code_Html($latex,$linenumbers=False)
    {
        $latex=preg_split('/\n/',$latex);
        $n=1;
    ...
    function Latex_Code_Show($latex,$linenumbers=True,$exit=True)
    {
        $this->Htmls_Echo
        (
            $this->Htmls_DIV
            (
                ...
                (
                    "",
                    $this->Latex_Code_Html($latex,$linenumbers)
                ),
                array
                (
                    "ALIGN" => 'left',
        ...
        if (!empty($this->ApplicationObj->LatexFilters))
        {
            foreach ($this->ApplicationObj->LatexFilters as $filter => $hash)
            {
                ...
                if (!empty($hash[ "Object" ]))
                {
                    ...
                    elseif (isset($this->ApplicationObj->$obj))
                    {
                        $this->ApplicationObj->$obj->DatasRead=array_keys($rfilter);
                        $rfilter=
                            $this->ApplicationObj->$obj->
    ...
    function Latex_Indent($latex,$w1,$w2,$pre="0.25cm")
    {
        if (is_array($latex))
        {
            $latex=join("",$latex);
