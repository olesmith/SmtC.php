...
{
    //console.log("Load_URL_2_Element_Do",cell_id,url);
...
function Load_URL_2_Element(cell_id,group,url,module11,debug=false)
{
    ...
    xhttp.onreadystatechange = function()
    {
        if (this.readyState == 4 && this.status == 200)
        {
            ...
            if (!element)
            {
                console.log("Element "+cell_id+" not found - add to ModuleCell");
            ...
            if (element)
            {
                ...
                Add_Exec_Time_To_Reload_Icon(element,date);
...
function Add_Exec_Time_To_Reload_Icon(element,date)
{
    //Add execution time to reload icon
    let reloads=element.getElementsByClassName('Reload');
    if (reloads.length>0)
    {
        let reload=reloads[0];

        reload=reload.parentElement;
        let rdate = new Date();
        let diff=rdate-date;
        diff=diff/1000;
...
function Load_JS(js)
{
    ...
    let date=new Date();
    scr.src=js+"?Random="+date.getTime();
    ...
    if (element)
    {
        let res=element.appendChild(scr);
        if (!res)
        {
        ...
        {
            console.log("Loaded",scr.src);
        }
    }
    else
    {
        console.log("No head or body",document.head);
    }
}
...
function Transfer_Loaded_Scripts(element)
{
    ...
    for (let n=0;n<scripts.length;n++)
    {
        ...
        if (scr)
        {
            ...
            let res=document.body.appendChild(scr);
            if (!res)
            {
...
function Load_Select(url,destination_id,cginame,source_id,debug=false)
{
    ...
    console.log(url,index,cgivalue);
...
function Load_Interface(is_mobile=false)
{
}

function Load_Cell(id)
{
    let url=Url2Hash(window.location.href);
    ...
    Load_URL_2_Element_Do(id,url);
}


//var json=false; ????
function Load_JSON0000(url,hash)
{
    let xhttp = new XMLHttpRequest();
    ...
    xhttp.onreadystatechange = function()
    {
        if (this.readyState == 4 && this.status == 200)
        {
            let json_in=this.responseText;
            
            hash=JSON.parse(json_in);
            //console.log(json);
        }
    }
