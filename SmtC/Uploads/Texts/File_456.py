...
def Vector_Latex(v,title="",env="pmatrix*",frmt="%.6f",sub="",options="r"):
    if (sub): sub="_"+sub
...
def Vectors_Sub(v,w):
    if (   len(v)!=len(w)   ):
        print ("Vectors_Sub: Vectors does not have same length")
        exit()
...
def Vectors_Angle(v,w):
    return Vectors_Dot(v,w)/(   Vectors_Len(v)*Vector_Len(w)   )
